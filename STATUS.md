...
- `servedAtMs` is the seller's own clock on the unilateral path. The bond is what
  ...
  measurement.
- Chainstack and Cencori were wired once, then dropped. Paid RPC and a quote
  gateway are not the product. Reads stay on Forno. `/api/rate` serves the
  published mid. Do not re-add either.
